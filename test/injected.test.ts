...
describe('Injected Script Logic (Main World)', () => {
  ...
  function waitForResponse(messageId: string): Promise<any> {
    return new Promise(resolve => {
      const handler = (e: MessageEvent) => {
        if (e.data.type === 'ACTUAL_BRIDGE_DATA' && e.data.messageId === messageId) {
  ...
  it('infers account ID from URL in Single Account view', async () => {
    ...
    expect(mockOnAdd).toHaveBeenCalledWith(expect.arrayContaining([
      expect.objectContaining({ account: accId })
    ]));
  }, 10000);

  it('prevents race conditions on concurrent imports', async () => {
    createMockTable();
    window.history.pushState({}, '', '/accounts/8f3b9c20-1234-1234-1234-123456789abc');

    // Simulate slow database
    mockOnAdd.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 50)));
    ...
    const skips = [res1, res2].filter(r => r.payload?.status === 'skipped_duplicate').length;
