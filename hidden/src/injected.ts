...
const TARGET_ORIGIN =
  (window as unknown as { __ACTUAL_BRIDGE_TEST_ORIGIN__?: string })
    .__ACTUAL_BRIDGE_TEST_ORIGIN__ || "__ACTUAL_ORIGIN__";

if (TARGET_ORIGIN === "__ACTUAL_" + "ORIGIN__") {
  const err =
    "Actual Bridge Security Error: Target origin not configured. You must pass { baseUrl } to bridge.connect().";
  console.error(err);
  throw new Error(err);
}
...
function findActual() {
  ...
  while (fiber && steps < 50) {
    ...
    if (p) {
      ...
      const hasHandlers =
        typeof p.onSave === "function" && typeof p.onAdd === "function";
...
async function resolvePayee(name: string, props: ActualProps) {
  ...
  const creator =
    props.onCreatePayee || (props.actions && props.actions.createPayee);
  ...
  if (typeof creator === "function") {
    ...
    try {
      ...
      pendingPayeeResolutions.delete(key);
    }
  }
...
window.addEventListener("message", async (event) => {
  ...
  if (!props) {
    reply(
      msg,
      undefined,
      "Actual Bridge: Not connected to UI. Are you on the Account screen?",
    );
    return;
  }
    ...
    if (msg.command === "GET_TRANSACTIONS") {
      ...
      if (!payload.account) {
        ...
        if (context.type === "SINGLE_ACCOUNT" && context.accountId) {
          ...
          throw new Error(
            "Cannot infer Account ID (Ambiguous View). Payload must include 'account'.",
          );
        }
      }
      ...
      if (
        payload.imported_id &&
        (importedIdSet.has(payload.imported_id) ||
          pendingImports.has(payload.imported_id))
      ) {
