import type { BridgeMessage, AnyBridgeResponse, Transaction, ProtocolMap, BridgeCommand, BridgeState, BridgeBroadcast } from './types';
...
export class ActualBridge {
  private listeners = new Map<string, { resolve: ResolveFn; reject: RejectFn }>();
  ...
  public async connect(config?: { baseUrl?: string }): Promise<ProtocolMap['HANDSHAKE']['res']> {
  ...
  public async getTransactions(): Promise<ProtocolMap['GET_TRANSACTIONS']['res']> {
    return this.sendCommand('GET_TRANSACTIONS', undefined);
  }

  public async saveTransaction(transaction: Transaction): Promise<ProtocolMap['SAVE_TRANSACTION']['res']> {
  ...
  public async importTransaction(payload: ProtocolMap['IMPORT_TRANSACTION']['req']): Promise<ProtocolMap['IMPORT_TRANSACTION']['res']> {
    if (!payload.imported_id) throw new Error("imported_id is required for deduplication.");
    return this.sendCommand('IMPORT_TRANSACTION', payload);
  }

  public async splitTransaction(originalTx: Transaction, splits: Partial<Transaction>[]): Promise<ProtocolMap['SAVE_TRANSACTION']['res']> {
